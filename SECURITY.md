...
Authentication is an additional layer, not permission to expose the service publicly. TiviMate playlist and EPG endpoints deliberately do not use the browser session; their long random output token remains the access control and can be revoked independently.
